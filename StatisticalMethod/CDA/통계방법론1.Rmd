...
-   Part2 Statistical Method - 요약, 비교, 관계
    -   통계적 의사 결정 Statistical Decision Making
    -   확률 분포와 그 응용
    -   확증적 비교 방법 Confirmatory Comparisons
...
-   설명 변수들간 선형 종속 관계일 때 다중 공선성이 존재한다.
    -   왜 문제인가?
...
-   ML 적용
    ...
    -   Multiple Comparison: 단순 2모델 비교가 아닐 때는 Multiple Comparison
        -   본페로니 보정을 통해서 더 엄격한 $\alpha$값을 설정
$$\frac{\alpha}{모델 개수}$$
...
-   (모집단의 특성에 대한) 어떤 주장이나 추측을 가설로 설정하고 표본 관찰을 통하여 **[이의 기각(rejection) 여부]{style="color: red;"}**를 결정하는 방법
-   통계적 가설
...
-   연속형 확률밀도 함수
    -   모든 가능한 y값에 대하여 확률 밀도 값은 0보다 크거나 같다.
    -   모든 가능한 y값에서 구한 면적은 1이다.
    -   관심 사상이 정의된 구간의 면적으로 해당 확률을 계산한다.

---

### 2.2.1. 정규분포 $Y$ ~ $N(\mu,\sigma^2)$

Normal Distributions

-   언제 사용하는가?
    -   구간/비율 척도로 측정되는 프로세스의 관심 특성에 대한 특징 파악
    -   좌우대칭, 종형의 형태를 갖는 경우
    -   제조 현장에서 목표값/참값이 명확하고 우연 변동(White Noise)만 존재하는 경우
-   정규 분포의 모수(들)는 무엇인가?
    -   $\mu: 모평균$
    -   $\sigma^2:~분산(\sigma:~표준편차)$
-   정규 분포의 유용성
    -   **[형태]{style="color: red;"}**, 좌우대칭,종형: 이상 원인(Special Cause)의 유무를 확인
    -   **[위치]{style="color: red;"}**, 평균: 목표값의 달성 여부 확인
    -   **[척도]{style="color: red;"}**, 분산/표준편차: 프로세스 관리 기술 수준에 대한 정보 제공
        -   분산/표준편차가 크다면, 관리 기술 수준이 낮은 것이다.
-   정규 분포의 표준화
    -   모든 정규 분포는 표준화를 통하여 표준 정규 분포로 변환될 수 있다.
    -   표준화의 장점
        -   (1) 표준 정규 분포를 이용한 확률 계산 용이
        -   (2) 서로 다른 측정 단위 또는 산포를 갖는 특성의 비교
        
$~~~~~~~~~~~~~~~~Y$ ~ $N(\mu,\sigma^2)이면,~~Z=\frac{Y-\mu}{\sigma}$~$N(0,1)$

---

## 실습 3. 이항 분포를 이용한 확률 계산

-   지난 분기 개발 업무(A)의 일정 미준수율은 약 20%정도이고 검증 업무 (B)의 시간 미준수율은 약 10%정도이다. 차기 분기 이전에 각각의 업무에서 임의로 선택된 30건에 대한 미준수 건수를 조사하여 개선 프로세스 도입 여부를 판단하고자 한다. (A)와 (B) 각 업무에 대하여 다음을 계산하시오.
    -   30건 중에서 미준수가 1건도 없을 확률
        -   (A)의 경우: $P(X=0) = \binom{30}{0} (0.2)^0 (0.8)^{30}$
        -   (B)의 경우: $P(X=0) = \binom{30}{0} (0.1)^0 (0.9)^{30}$
    -   30건 중에서 미준수가 6건 이상 발생할 확률
        -   (A)의 경우: $1-(P(X=0)+P(X=1), ..., +P(X=5))$
        -   (B)의 경우: $1-(P(X=0)+P(X=1), ..., +P(X=5))$
    -   미준수 건수가 4건 이상일 확률이 50%를 넘지 않는다면 현재 프로세스를 유지하고, 만일 50%를 넘는다면 새로운 관리 기준을 도입하려 한다. 두 업무는 새로운 프로세스를 도입해야 하는가 여부를 결정하시오.
...
-   A 업무는 처리 시간(분)이 중요 특성이며 이는 $N(30, 3^2)$을 따른다. 또한 만일 처리 시간이 35분을 넘는 경우 고객 불만족이 발생하는 것으로 알려져 있다.
...
## 실습 7. 범주형 데이터의 Wrangling/Munging : C-T-F
...
## 실습 13. 정규성 변환(Box Cox Transformation)
...
-   p-value = 0.0000000000000002 < $\alpha = 0.05$ 이므로 $H_{0}$를 기각한다.
    -   데이터는 정규성을 만족하지 않는다.
    -   BOX-COX 변환 시도
...
```
-   p-value = 0.8 > $\alpha = 0.05$ 이므로 $H_{0}$를 기각하지 못한다.
    -   데이터는 정규성을 만족한다.
...
-   p-value > $\alpha = 0.05$ 이므로 $H_{0}$를 기각하지 못한다.
    -   따라서 데이터는 정규성을 따른다.
...
## 실습 15. 소음 개선 후 Pilot Sample 개선 검증
...
## 실습 16. 쌍체 데이터의 차이의 모평균 비교

-   exposure.con.csv
-   노출 시간은 표본 변이에 민감하며, 결과인 집적도(Concentration;C/m^2)에 영향을 준다.
...
-   따라서, 노출 시간 3 min.s일 때와 1 min.s를 비교 했을 때 집적도가 2.0(C/m^2)이상 차이가 난다는 것은 통계적으로 유의하다.
...
-   오차는 상호 독립이다.
-   오차는 정규 분포(normal distribution)를 따른다.
-   오차는 모든 설명 변수의 값에 대하여 분산 \(\sigma^2\)가 같다.
...
## 실습 17. 임의로 선택된 소형 화물의 무게$(x;~g)$과 부피$(y;~cm^3)$
...
-   신뢰대는 빨간색 점선으로 이루어져 있다.
    -   노란색으로 색칠된 데이터가 위 데이터의 평균을 의미하고 평균을 기준으로 95%에 대한 신뢰구간이다.
...
-   00유통에서는 물품의 수에 따른 매장 정리 시간의 기준을 결정하기 위하여 다음의 조사를 실시하였다.
    -   일상적인 정리업무를 하는 직원 중에서 랜덤하게 15명을 선택
    -   각 직원 별 랜덤하게 할당된 서로 다른 물품의 수를 정리하는데 소요되는 정리 시간(min.s)을 기록
...
-   모형 적합 후 표준화 잔차와 적합치를 보완한 뒤에 잔차 분석`m_residuals(df)`을 실행
...
-   예측대 내에 데이터가 잘 들어와 있는 것으로 모아 모델이 잘 적합되어 있다.

---

## 실습 19. Test Case Determination
...
-   00전자의 SWD팀은 최근 개발된 모델들을 중심으로 "KLOC"에 따른 적절한 'Test Case'의 수에 대한 기준을 결정하기 위하여 기존 개발된 모델에서 두 변수간 밀접한 관계가 있는지를 확인하고, 이에 대한 모형을 설정하고자 한다.
    -   각 변수의 결측값(NA)의 수를 확인하시오.
    -   단순 회귀 모형을 가정하고 회귀 계수의 유의성 검정 결과를 제시하시오.
    -   결정 계수의 값을 제시하시오.
    -   잔차 진단을 실시하고 이상점 유무를 확인하시오.
    -   신뢰대와 예측대를 포함하여 단순 회귀 분석의 결과를 요약하시오.
...
-   일부 양끝단을 제외하고는 정규성을 만족하는 것으으로 보인다.
...
-   p-value = 0.0000000000000002 < $\alpha = 0.05$ 이므로 $H_{0}$를 기각하므로 통계적으로 유의하다.
    -   위 회귀모형은 적절하다.
-   신뢰대는 빨간색 점선으로  회귀선의 평균값에 대한 신뢰도를 나타낸다.
    -   이는 회귀선의 평균값이 얼마나 정확한지를 의미한다.
...
-   다중 회귀 분석: **[lm()]{style="color: blue;"}*
    -   **[lm()]{style="color: blue;"}* 함수는 단순 회귀 분석과 동일하게 적용된다.
...
-   전방 선택법, 후방제거법, 단계별 선택법, 모든 부분 집합법
